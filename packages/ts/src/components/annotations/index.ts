...
// Local Types
import { AnnotationDatum, AnnotationSubject } from './types'

// Config
import { AnnotationsConfig, AnnotationsConfigInterface } from './config'

// Styles
import * as s from './style'

export class Annotations<Datum extends AnnotationDatum> extends ComponentCore<Datum[], AnnotationsConfig<Datum>, AnnotationsConfigInterface<Datum>> {
  static selectors = s
  static cssVariables = s.variables
  config: AnnotationsConfig<Datum> = new AnnotationsConfig()
  g: Selection<SVGGElement, unknown, null, undefined>

  events = {}

  constructor (config?: AnnotationsConfigInterface<Datum>) {
    super()
  ...
  _render (customDuration?: number): void {
    ...
    const annotations = this.g
      .selectAll<SVGGElement, Datum>(`.${s.annotation}`)
      .data(data ?? [])
