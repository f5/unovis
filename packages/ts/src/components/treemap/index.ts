...
export class Treemap<Datum> extends ComponentCore<Datum[], TreemapConfigInterface<Datum>> {
  ...
  _render (customDuration?: number): void {
          ...
          if (hexColor) {
            ...
            if (config.enableLightnessVariance) {
              if (!treemapChild.children) {
                const lightnessAdjustment = this._getTileLightness(treemapChild, children)
    ...
    mergedTiles.select('g.label-group text')
      .text(d => `${d.data.key}: ${numberFormat(d.value)}`)
    ...
    smartTransition(mergedTiles.select('g.label-group'), duration)
      .attr('transform', d => `translate(${d.x0 + config.labelOffsetX},${d.y0 + config.labelOffsetY})`)

    // Transition text opacity only (fade-in)
    smartTransition(mergedTiles.select('g.label-group text'), duration)
      .style('opacity', 1)
