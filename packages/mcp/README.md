...
Fonts: drop `Inter-Regular.ttf`/`Inter-Bold.ttf` (or any fonts) into `fonts/` to register them for text measurement — Unovis defaults to Inter; without font files, system fonts are used for measurement (metrics are close, not exact).
