 ...
 *
 * Runs against the BUILT dist (pnpm build first — the `samples` script chains
 * it): the tsx loader hangs on dynamic imports inside @unovis/ts graph
 * layouts, while plain node handles them fine.
 *
 * Run: pnpm samples
 */
...
import { recipes, renderChart, svgToPng, themeBackground } from '../dist/index.js'
...
const fixtureName = (recipeFile) => recipeFile.replace(/_/g, '-')
...
for (const recipe of recipes) {
  ...
  // Fixture .ts files are data-only (erasable syntax) — Node ≥23.6 imports
  // them natively via type stripping, no loader needed
  const samples = (await import(pathToFileURL(fixtureFile).href)).default
  ...
  for (const sample of samples) {
    for (const theme of ['light', 'dark']) {
      ...
      try {
        ...
        cards.push(`<figure class="${theme} error"><figcaption>${label} 💥 ${e.message}</figcaption></figure>`)
      }
    }
  }
}
...
// Lingering d3 timers keep the event loop alive after rendering — exit explicitly
