...
The XYContainer is designed as a wrapper to hold other `vis` elements. Here is an example of a common congifuration:
an `xy-chart` alongside two `axis` components:

```angular2html
<vis-xy-container [data]="data">
  <vis-stacked-bar [x]="x" [y]="y"></vis-line>
...
It accepts type `[number, number]` where the the elements represent the starting and ending positions, respectively.
...
To change the scale of one of your axes, use the `xScale` or `yScale` property and a `Scale` function. (i.e. `Scale.scaleLinear()`) The following optoins are available:

`scaleLinear`, `scalePow`, `scaleSqrt`, `scaleLog`, `scaleIdentity`, `scaleTime`,
`scaleUtc`, `scaleSequential`, `scaleDiverging`, `scaleQuantize`, `scaleQuantile`,
...
See an example of the a simple `y = x` line with an xScale property set to `Scale.scaleSqrt()`
...
See an example of the a simple `y = x` line with an yScale property set to `
