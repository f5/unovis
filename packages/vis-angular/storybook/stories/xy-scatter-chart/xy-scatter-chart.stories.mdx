...
Consider this example, where a point's color depends on its category.
```javascript
...
### Point Shape
Similarly, you can change the point's symbol using the `symbol` attribute, which accepts a `SymbolType` or string.
```javascript
...
```angular2html
<vis-scatter ... [cursor]="crosshair"></vis-line>
```
