...
import { AxisType, Position, TrimMode, FitMode, TextAlign } from '@volterra/vis';
...
import { xyAxisWrapper } from '../../utils/content-wrappers'
import { baseConfig, baseXConfig, baseYConfig, withDataConfig, withTimeDataConfig, multiAxisConfig, multiAxisStory } from './xy-axis.config'
...
Like with other xy-components, you can provide xAccessor and yAccessors to populate the axis values. Consider the following example with a
dataArray and a single axis and xAccesor function:
...
### Axis Padding
If you want to change the distance between the axis and chart, you can change the `padding` property on the Axis component. See the same data
used in the previous example but with a custom `Spacing` given to `padding`.

```angular2html
// In the component
const axisPadding = { top: 50 }

// In the template
<vis-axis ... [tickValues]="axisPadding"></vis-axis>
```

<Story name="Axis Padding">
  {{
    props: {
      ...withDataConfig(10, "X"),
      y: d => d.x,
