...
const DataReactivity = (): JSX.Element => {
  const [numPoints, setNumPoints] = createSignal(DEFAULT_NUM_POINTS)
  const [seed, setSeed] = createSignal(0)
  const [lineWidth, setLineWidth] = createSignal(DEFAULT_LINE_WIDTH)
  const data = createMemo(() => generateData(numPoints(), seed()))
