...
- Provide your own functions to render nodes allowing you to highly customize how the graph looks ([docs](/docs/components/Graph#custom-rendering-15)).

- Post-Layout ([docs](/docs/components/Graph#post-layout-customization-15)) and Post-Render Customization ([docs](/docs/components/Graph#post-render-customization-15)) allowing you to modify the layout of the graph on the fly and render additional layers with D3.

- Provide custom SVG icon to link labels ([docs](/docs/components/Graph#labels-updated-in-15)).

- Zoom start/end and node dragging callbacks ([docs](/docs/components/Graph#pan--zoom--drag)).

- Fit view to specific nodes by providing an array of node ids.

- Multiple node selection ([docs](https://unovis.dev/docs/components/Graph#multiple-node-drag)).

- Enable _Graph_ nodes to accept precalculated layout data  ([docs](https://unovis.dev/docs/components/Graph#precalculated)).
...
*Tooltip* now can be anchored to the target element, can be hovered over, and supports dynamic content (updates if the content changes) ([docs](/docs/components/Tooltip#follow-cursor)).
...
Axis now automatically hides overlapping labels ([docs](/docs/components/Axis#hide-overlapping-ticks-15)) and supports label rotation ([docs](/docs/components/Axis#tick-label-rotation)).

### 🔵 Bullet Legend
You can set the orientation of _Bullet Legend_ to `'vertical'` ([docs](/docs/components/BulletLegend#orientation)).
