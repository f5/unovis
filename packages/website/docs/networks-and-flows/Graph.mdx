...
<GraphDoc nodeFill={getRandomColor} nodeStroke={getRandomColor} nodeStrokeWidth={(d, i) => 1 + i * 3}/>
...
<GraphDoc nodeGaugeFill={(d, i) => gradient(i * 10)} nodeGaugeValue={(d, i) => i * 10} />
...
and provide the `layoutNodeGroup` accessor function.
