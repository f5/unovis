...
#### Adaptive Node Padding
When _SingleContainer_ uses the default `Sizing.Fit`, a column with many nodes can force the combined
`nodePadding` to exceed the available height, collapsing node bodies to near-zero height. Set
`nodeAdaptivePadding` to `true` to automatically compress `nodePadding` in that case, so that node bodies
always retain at least a third of the available height. The padding is only reduced when it doesn't fit;
otherwise the configured `nodePadding` is used as-is. This option has no effect for `Sizing.Extend` or
`Sizing.FitWidth`. Default: `false`.
