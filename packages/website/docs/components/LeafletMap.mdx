...
## MapLibre worker loading
With the MapLibre renderer, tiles are parsed off the main thread in a Web Worker. Bundlers can't reliably
resolve MapLibre's own worker file via `import.meta.url`, so by default _LeafletMap_ starts the worker from
a `Blob` URL of a prebuilt worker bundled with _Unovis_ (`useMaplibreWorkerBlob: true`). This works out of
the box with every bundler and doesn't require any extra configuration, but does require `worker-src blob:`
in your [CSP](../guides/csp#recommended-csp-header) if you enforce one.

Set `useMaplibreWorkerBlob: false` if you'd rather serve MapLibre's actual worker file yourself (e.g. to
avoid `blob:` in your CSP, or to keep the worker version pinned to whatever `maplibre-gl` build you install).
When disabled, _LeafletMap_ won't set a worker URL at all, so you must:

1. Copy `node_modules/maplibre-gl/dist/maplibre-gl-worker.mjs` into a folder your app serves as a static
   asset (its name doesn't matter, but it must be reachable by URL at runtime).
2. Call MapLibre's `setWorkerUrl()` with that URL yourself, **before** the map is initialized.

<Tabs groupId="framework">

<TabItem value="react" label="React">

**Vite** — static files placed in `public/` are served as-is from `/`:

```ts
// main.tsx, before rendering the LeafletMap component
import maplibregl from 'maplibre-gl'

maplibregl.setWorkerUrl('/maplibre-gl-worker.mjs')
```

Copy the file into `public/maplibre-gl-worker.mjs` (e.g. via a `postinstall`/`prebuild` script, or a
Vite plugin such as `vite-plugin-static-copy`).
...
```ts
// main.ts, before bootstrapping the app
import maplibregl from 'maplibre-gl'

maplibregl.setWorkerUrl('/assets/maplibre-gl-worker.mjs')
```

```json title="angular.json — copy the worker file into the build output"
"assets": [
  { "glob": "maplibre-gl-worker.mjs", "input": "node_modules/maplibre-gl/dist", "output": "/assets" }
]
```

This applies the same way whether Angular serves your app with its Vite-based dev server or bundles it
with esbuild for production — both just need the file to exist at the `assets` path you copied it to.

</TabItem>

<TabItem value="svelte" label="Svelte / SvelteKit">

Files in SvelteKit's `static/` directory are served from `/`:

```ts
// hooks.client.ts, or any module that runs before the map component
import maplibregl from 'maplibre-gl'

maplibregl.setWorkerUrl('/maplibre-gl-worker.mjs')
```

Copy the file into `static/maplibre-gl-worker.mjs` as part of your build (e.g. a small script run
before `vite build`, or in `vite.config.ts`'s `buildStart` hook).

</TabItem>

<TabItem value="vue" label="Vue">

Same as React's Vite setup — copy the file into `public/maplibre-gl-worker.mjs` and set the URL before
the map initializes:
...
Same pattern — copy the file into `public/maplibre-gl-worker.mjs` and set the URL in your entry point
before the map component mounts:
...
```ts
// app.ts, before creating the LeafletMap instance
import maplibregl from 'maplibre-gl'

maplibregl.setWorkerUrl('/maplibre-gl-worker.mjs')
```

Serve `maplibre-gl-worker.mjs` from your static file host alongside the rest of your app.
