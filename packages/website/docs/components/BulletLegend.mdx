...
Note that there is no specified default value unless `onLegendItemClick` property is provided, in which case the
cursor will be `pointer`.

## Orientation
The `orientation` property can be set to `BulletLegendOrientation.Horizontal` (`'horizontal'`) or
`BulletLegendOrientation.Vertical` (`'vertical'`) to change the layout of the legend items.
