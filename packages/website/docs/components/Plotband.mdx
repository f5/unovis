...
When `labelAutoPosition` is `true` and a label collides with others, `labelOverflow` controls how the conflict is resolved:

* `stack` *(default)* – shift the label along the perpendicular axis just enough to clear the collision. Label stays visible at its preferred anchor, displaced by the overlap amount.
...
The example below renders eighteen overlapping plotbands. Switch `labelOverflow` to see the highlighted **MAIN BAND** (red) label displaced (*stack*) or hidden (*hide*).
