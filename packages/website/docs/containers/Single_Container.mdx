...
import { sankeyData, orderedData } from '../utils/data'
...
height isn't defined, the defaul height of `300px` will be applied. You can also explicitly define the container's
...
<div className='center'>
  <BrowserOnly>
  {() => {
    const { VisSingleContainer, VisTopoJSONMap } = require('@unovis/react')
...
### Margin
You can set _Single Container_'s `margin` to control the spacing between the container and adjacent
