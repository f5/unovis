...
The payload is the same `SingleContainerRenderPayload` for all three:

```ts
type SingleContainerRenderPayload = {
  svg: SVGSVGElement;
  margin: Spacing;
...
```vue
<!-- Vue -->
<VisSingleContainer :data @load="onLoad" @render="onRender" @redraw="onRedraw" />
```

```svelte
<!-- Svelte -->
<VisSingleContainer {data} on:load={onLoad} on:render={onRender} on:redraw={onRedraw} />
```

```html
<!-- Angular -->
<vis-single-container [data]="data" (load)="onLoad($event)" (render)="onRender($event)" (redraw)="onRedraw($event)" />
```

For React and Solid, use the `onRenderComplete` config callback. It receives the same fields as positional args: `(svgNode, margin, containerWidth, containerHeight, componentWidth, componentHeight)`.
