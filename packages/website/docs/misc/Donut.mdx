---
sidebar_class_name: vis-icon__donut
...
the `centralSubLabelWrap` property to `false`), while the main label is supposed to be short and doesn't have wrapping
  implemented.
<DocWrapper {...defaultProps()} centralLabel="Label" centralSubLabel="Long sub-label wraps onto the next line"/>
...
<InputWrapper {...defaultProps()} data={[0]} angleRange={[-Math.PI / 2, Math.PI / 2]} property="showBackground" inputType="checkbox" defaultValue={true}/>
...
<DocWrapper {...defaultProps()} data={[1,2,3]} showBackground angleRange={[0, Math.PI / 3]} backgroundAngleRange={[0, 2 * Math.PI]} />
