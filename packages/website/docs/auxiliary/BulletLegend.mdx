...
Note that the only requred property, `name` corresponds to the legend items' label.
...
### Custom Colors
By default, our [color palette](docs/guides/theming#color-palette) will be used to color each legend item, but
but you can provide your own colors in the legend item array.
```ts
const colors = ['red', 'blue', 'green']
const items = labels.map((label, i) => ({ name: label, colors[i] }))
```
...
function toggleItem (item: BulletLegendItemInterface, i: index): void {
  ...
  legend.update({ ...legend.config, items: itemsCopy })
