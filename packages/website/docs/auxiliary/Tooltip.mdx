...
## Hoverable Content
You can make _Tooltip_'s content hoverable by setting `followCursor` property to `false`.
This will keep the content in view when hovering over the tooltip itself.
This can be useful for interacting with text, links, or other elements in the tooltip:

Here is an example with a _Line_ chart a clickable link on hover:

<Tooltip {...defaultProps("Line", { lineWidth: 4 })}
  followCursor={false}
  triggers={{
