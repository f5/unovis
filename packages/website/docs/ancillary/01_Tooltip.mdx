...
`container` property. In the most cases you might want to set `container` to `document.body` and `positionStrategy`
to `PositionStrategy.Fixed` (see below).


### Position Strategy
The `positionStrategy` property defines how the tooltip will be positioned relative to its container. Supported values
are `PositionStrategy.Absolute` (or just `"absolute"`; default value) and  `PositionStrategy.Fixed` (or just `"fixed"`).
The values correspond to the [CSS position](https://www.w3schools.com/css/css_positioning.asp) property.

When you manually set the `container` property of your tooltip to `document.body` or some other DOM element, it's recommended
to set `positionStrategy` to `PositionStrategy.Fixed` to avoid misplacement.
<BrowserOnly fallback={<div>Loading...</div>}>
  {() => <Tooltip {...defaultProps("StackedBar", {}, 50)} container={document.body} positionStrategy="fixed" height={40} />}
</BrowserOnly>
