...
import { XYWrapper, axis } from '../xy-charts/wrapper'
...
export const paletteData = data.map(d => ({
  ...d,
...
to supply the values of various SVG attributes such as `fill`, `stroke`, `opacity`, etc. You can
use override these variables to further customize your _Unovis_ components.
...
## Basic Example
Variables can be overriden in your CSS style declarations. Consider the default configuration for sankey, which looks like this:
...
## Global Variables
They majority of our variables exist on a component level,  but there are a few global CSS variables:

<CSSVariables selector=':root' vars={['--vis-font-family', '--vis-color-main', '--vis-color-gray']}/>
...
The full pallete looks like this:
...
You can do this by maniuplating the `stroke` and `stroke-width` variables to create a bordered segment
effect.
...
<div className='border-lines'>
  ...
  <h4>Donut</h4>
  <DocWrapper name='Donut' containerName='SingleContainer' {...borderLineProps} data={Array(10).fill(0)} value={() => Math.random()} arcWidth={50}/>
  <h4>Stacked Bar</h4>
