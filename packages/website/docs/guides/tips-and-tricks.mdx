...
We don't natively support the ordinal scale for XY components, but it can still be acheived with some
small tweaks.
...
```
  **Case 2**: Your ordinal values are explitly defined as an array of strings:
