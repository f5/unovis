...
- **`worker-src 'self' blob:`** — only required if you render the
  [ELK-layered graph](../components/Graph) or the
  [Leaflet Map](../components/LeafletMap) with the MapLibre renderer; both
  spawn a Web Worker from a `Blob` URL.
...
- **Web Workers from `Blob` URLs** — the ELK-layered graph and the MapLibre
  renderer of the Leaflet Map use one. Add `worker-src 'self' blob:` if you
