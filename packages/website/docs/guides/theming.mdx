...
import { CSSVariables } from '../doc-components/css-variables'
...
import { sankeyProps as sankeyProps } from '../components/Sankey.mdx'
import { donutProps as donutProps } from '../components/Donut.mdx'
import { FrameworkTabs } from '../doc-components/framework-tabs.tsx'
...
`--vis-area-cursor` would apply to the [Area](../components/Area) component's cursor property.
...
If the [`labelPosition`](../components/Scatter#label-position) property is set to `Position.Center`,
point labels will try fit to the point's size. In this case, you will instead need to update the
[`pointSize`](../components/Scatter#size-and-size-range) property to render larger labels.

- [**Timeline**](../components/Timeline)
Additionally, you may need to adjust the [`rowHeight`](../components/Timeline#row-height) property to
