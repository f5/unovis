...
import { XYWrapper } from './xy-charts/wrapper'
import { FrameworkTabs } from './components/framework-tabs.tsx'


## Custom Fills with `SVG defs`
If you want to custom `svg` defintions available for your components, you can use the `svgDefs` property on the container, like this:
...
Alternatively, you can define any defs you want to use in a `svg` element on the same page.
