...
export const TopoDoc = (props) => (
  <BrowserOnly>
  {() =>  {
      const lib = require('@volterra/vis/maps')
      const imports = {
        "@volterra/vis/maps": [props.topojson],
      }
      if (props.showData) imports['@volterra/vis'] = ["MapData"]
      return (
...
where `MapArea`, `MapPoint`, and `MapLink` are custom types that that represent map data. Keep
...
```
Or, you can simply provide these values through the `latitude` and `longitute` properties of your
...
<TopoDoc
  {...defaultProps()}
  dataType="MapPoint,MapLink,undefined"
  data={{...data, areas: [] }}
  showContext="container"
/>
...
<TopoDoc
  ...
  dataType="MapArea,undefined,undefined"
  pointId={d => d.id}
  showContext="minimal"
/>
...
<BrowserOnly>
{() => {
  const { MapProjectionKind, MapProjection } = require('@volterra/vis')
  const { WorldMapTopoJSON } = require('@volterra/vis/maps')
  const imports = {
    "@volterra/vis": ["MapData", "MapProjection"],
    "@volterra/vis/maps": ["WorldMapTopoJSON"],
  }
...
<TopoDoc {...defaultProps()} zoomFactor={5} zoomExtent={[1,5]}/>
...
In order for your the _TopoJSONMap_ component to render propertly, the `topojson` property and a
...
<table style={{ display: 'table', margin: '0 auto', width: '100%' }}>
  ...
  <tbody>
    ...
    ].map(([name, feature, url, link, example]) => (
      <tr>
      <td>{name}</td><td>{feature}</td>
      <td>
...
## Events
```ts
import { TopoJSONMap } from '@volterra/vis'
