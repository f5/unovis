...
export const defaultProps = (showData = true) => ({
  ...
  visImports: showData ? ["WorldMapTopoJSON", "MapData"] : ["WorldMapTopoJSON"],
...
<DocWrapper {...defaultProps(false)} showContext="container"/>
...
<DocWrapper {...defaultProps()} data={pointData} topojson="WorldMapTopoJSON" dataType="MapPoint,undefined,undefined" showContext="container"/>
...
<DocWrapper {...defaultProps()} data={{ points: data.points }} dataType="MapPoint,undefined,undefined" pointLabel={d => d.city} showContext="minimal"/>
...
## Areas
To work with features in the _TopoJSONMap_, all you need is a unique `id` which is defined in the chart's _topojson_ definition.
...
<DocWrapper {...defaultProps()} data={heatmapData} heatmapMode={true}/>
