...
import { data, generateDataRecords, generateTimeSeries } from '../utils/time-series'
import { parseProps } from '../utils/props-helper'
import { XYWrapper, XYDocTabs, XYWrapperWithInput, XYWrapperWithMenu } from '../components';
...
<XYDocTabs componentStrings={{
    ...
    typescript: `const components = [
    new Axis({ type: "x", position: "top", label: "X" }}),
    new Axis({ type: "y", position: "right", label: "Y" }})\n]`
}}/>
<AxesWrapper xConfig={{position: "top", label: "X"}} yConfig={{position: "right", label: "Y"}} showTabs/>

<small>Note: An X *Axis* can only accept the values `Position.Bottom` (default) and `Position.Top` and a Y *Axis* can only accept `Position.Left` (default) and `Position.Right`.
The default will be used in the case of an invalid position argument.</small>
...
### Label Margin
You can provide a string to label your axes with the label property:
<XYWrapperWithInput {...defaultProps()} label="Label" inputType="range" defaultValue={5} property="labelMargin"/>
...
If you use the Axis component alone, without other xy-components, you'll need to provide xAccessor or yAccessors to populate the axis values.
Consider the following example with a single axis and a dataArray with x values in the range `0 < x < 10`:

<XYWrapper {...defaultProps()} x={d => d.x} showContext/>
Another example:

```ts
const x = (data: DataRecord[]) => d.x * 100
```
...
### Tick Format
You can customize how ticks are formatted using the tickFormat attribute and a label formatter function.
The following example uses Javascript's built-in Date formatter function `toDateString()`.
<XYWrapperComponent {...defaultProps()} data={generateTimeSeries(10)} x={d => d.timestamp} tickFormat={d=> new Date(d).toDateString()}/>
...
<XYWrapperWithMenu {...defaultProps()} data={generateTimeSeries(10)} hiddenProps={{gridLine: false, x: d => d.timestamp, tickFormat: d=> new Date(d).toDateString()}} options={['right', 'center', 'left']} attr="tickTextAlign"/>
...
*Axis* accepts a `TrimMode` or a string. When we provide the previous example with TrimMode.End property, we can see the end of the label gets cut off instead of the middle.
<XYWrapperWithMenu
  {...defaultProps()}
  tickTextLength={10}
  data={generateTimeSeries(10)}
...
### Text Fit Mode
*Axis* accepts the following values for the tickTextFitMode property: `FitMode.Wrap` or `FitMode.Trim`. This determines how the axis will
...
### Text Separator
Axis accepts a string or string[] value fortickTextSeparator attribute. This will allow tick labels to be separated by custom values in the case of overflow.
...
By default, the Axis component provides an optimal the number of ticks displayed based on the component's size. You can alter the tick count to your liking using the numTicks property.
<XYWrapperWithInput
  {...defaultProps()}
  inputType="range"
...
### Custom Values
You can customize the ticks displayed by providing the *Axis* component with an number array.
...
You can include a chart within your *XYContainer* alongside your axes like this:
<XYDocTabs componentStrings={{
    react: `<VisAxis type="x"/>\n      <VisAxis type="y"/>\n      <VisLine x={x} y={y}/>`,
    angular: `<vis-axis type="x"></vis-axis>\n  <vis-axis type="y"></vis-axis>\n   <vis-line [x]="x" [y]="y"/>`,
    typescript: `const components = [
...
## Axis configuration within container
Alternatively, you can configure your X and Y *Axis* component within the container's configuration like this:
