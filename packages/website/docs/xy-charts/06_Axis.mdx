...
#### AxisType.Y
<XYWrapper {...defaultProps()} type="y" xyConfigKey="yAxis"/>

### Axis Position
You can change the position of your axis with a `Position` type or a string equivalent: `"top"`, `"right"`, `"bottom"`, or `"left"`.

<XYWrapper {...defaultProps()} position="top" label="X" componentProps={[{name: "Axis", key: "yAxis", props: {type: "y", position: "right", label: "Y"}}]} showContext="full"/>
