...
import { XYWrapper, XYWrapperWithInput, DynamicXYWrapper } from '../components/'
...
<XYWrapperWithInput {...defaultProps()} y={[d => d.y, d => d.y1, d => d.y2]} height={250} showAxes property="orientation" inputType="select" options={["vertical", "horizontal"]}/>
...
<DynamicXYWrapper {...defaultProps()} primaryData={generateDataRecords(3)} secondaryData={generateDataRecords(50)} exampleProps={{barMaxWidth: 40}}/>
...
<XYWrapperWithInput name="StackedBar"
    ...
    property="barMinHeight"  inputType="checkbox" showAxes/>
...
<XYWrapperWithInput
    ...
    defaultValue={true}
/>
<XYWrapperWithInput {...defaultProps()} y={d => d.y} property="roundedCorners" inputType="range" inputProps={{ min: 0, max: 30}} defaultValue={5}/>
...
In this example, each bar's color is assigned based on it's value:
<XYWrapper {...defaultProps()} showContext="minimal" showAxes color={(d,i) => d.y > 7 ? '#FF4F4E' : '#1acb9a'}/>
...
{`--vis-stacked-bar-cursor
--vis-stacked-bar-fill
--vis-stacked-bar-stroke
--vis-stacked-bar-stroke-width
--vis-stacked-bar-hover-stroke-width
--vis-stacked-bar-hover-stroke
`}</CodeBlock>
</details>

## Component Props
<PropTable name="VisStackedBar"></PropTable>
