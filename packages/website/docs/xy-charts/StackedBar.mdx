...
export const MyComponent = ({ type = "XYWrapper", n = 10, name = "StackedBar", x = (d) => d.x, y = (d) => d.y, data=generateDataRecords(n), ...customProps }) => {
  if(type==="XYWrapperWithInput") return (<XYWrapperWithInput {...{ name, data, x, y }} showContext="full" {...customProps} />)
  if(type === "DynamicXYWrapper") return (<DynamicXYWrapper {...{ name, data, x, y }} showContext="full" {...customProps} />)
  return (
    <XYWrapper {...{ name, data, x, y }} showContext="full" {...customProps} />
  );
};
...
<MyComponent y={[d => d.y, d => d.y1, d => d.y2]}/>

## Orientation
_Stacked Bar_ supports horizontal and vertical orientation.
<MyComponent type="XYWrapperWithInput" y={[d => d.y, d => d.y1, d => d.y2]} height={250} showAxes property="orientation" inputType="select" options={["horizontal","vertical"]}/>
...
<MyComponent excludeTabs y={[d => d.y, d => d.y1, d => d.y2]} showContext='container' color={['red', 'green', 'blue']}/>
...
```
<MyComponent excludeTabs showContext="minimal" showAxes={true} color={(d,i) => d.y > 7 ? '#FF4F4E' : '#1acb9a'}/>
...
<MyComponent type="XYWrapperWithInput" property="barWidth" inputType="number" defaultValue={5}/>
...
<MyComponent type="XYWrapperWithInput" property="barPadding" inputType="range" inputProps={{ min: 0, max: 1, step: 0.1}} defaultValue={0.5}/>
...
<MyComponent type="XYWrapperWithInput" name="StackedBar"
...
<MyComponent type="DynamicXYWrapper" showAxes secondaryData={generateDataRecords(5)} primaryData={generateDataRecords(10).map((d, i)=> ({ x: i < 5 ? Math.random() : i - 6, y: d.y }))} exampleProps={{dataStep: 0.1}}/>
...
```
<MyComponent excludeGraph events={{}}/>
...
```
<MyComponent excludeTabs className="custom-stacked-bar"/>
