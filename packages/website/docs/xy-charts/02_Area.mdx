...
import { generateDataRecords } from "../utils/time-series";
...
<XYWrapper {...defaultProps()} y={[(d) => d.y, (d) => d.y1, (d) => d.y2]} showContext/>

### Curve Types
Using the `curveType: CurveType` property you can set various curve type options. For example:
<XYWrapperWithMenu {...defaultProps(25)} options={["basis", "linear", "step"]} attr="curveType"/>
...
<XYWrapper {...defaultProps()} showContext y={[(d) => d.y, (d) => d.y1, (d) => d.y2]} color={(d, i) => ['#6A9DFF', '#1acb9a', '#f88080'][i]}/>
