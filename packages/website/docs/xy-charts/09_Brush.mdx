...
<BrowserOnly>
  {() => {
    ...
    function updateTooltip(selection, event, userDriven) {
      if (userDriven) {
        console.log(event)
        tooltip.show('hi', { x: 0, y: 0 })
      }
    }
...
<XYWrapper {...defaultProps()} draggable={true}/>

## Brush Appearance
### Handle Position
`handlePosition` changes the placement of the _Brush_ handle with respect to the _XYChart_.
<XYWrapperWithInput {...defaultProps()} property="handlePosition" inputType="select" options={['outside', 'inside']}/>
