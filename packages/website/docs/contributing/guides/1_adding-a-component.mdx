...
# Adding a new component to Unovis
...
We have three wrappers for our library for usage in other frameworks: `@unovis/angular`,
`@unovis/react`, `@unovis/svelte`. Each can be found in their named directory in `packages`.
...
- [x] It is exported by each wrapper library (`@unovis/angular`, `@unovis/react` and `@unovis/svelte`)
