...
this process: the wrappers are generated from a single shared registry. You will just need to do the following steps:

1. Add an entry for your component to the `getComponentList()` array in `packages/shared/integrations/components.ts`. This registry is the source of truth for all of the wrapper generators (it's also what validates `Component` commit scopes).
2. In each wrapper's root directory (`packages/react`, `packages/angular`, `packages/svelte`, `packages/vue`, `packages/solid`), run the following command:
  
  ```bash
  pnpm generate
  ```
3. Take a look at the generated component to see if it looks right. The Solid, Svelte and Vue barrels
(`src/components.ts`) are updated automatically; for React and Angular, add the export line to
`src/components.ts` by hand. Never edit the generated component files directly (they carry a
"do not change" banner) — if the output is wrong, fix the core config or the shared TS parser
(`packages/shared/integrations`) and regenerate.
